...
interface WidgetSelectedPaymentMethod<CustomPaymentMethodCode = WidgetsPaymentMethodCode> {
    /**
     * 구매자가 결제 UI에서 선택한 결제수단입니다. 예를 들어, 신용・체크카드를 선택하면 `CARD` 코드가 응답됩니다. [ENUM 코드](/codes/enum-codes) 및 [기관 코드](/codes/org-codes)를 참고하세요.
     */
    code: WidgetsPaymentMethodCode | CustomPaymentMethodCode;
}
type WidgetsPaymentMethodCode = 'CARD' | 'VIRTUAL_ACCOUNT' | 'MOBILE_PHONE' | 'TRANSFER' | 'CULTURE_GIFT_CERTIFICATE' | 'GAME_GIFT_CERTIFICATE' | 'BOOK_GIFT_CERTIFICATE' | 'TOSSPAY' | 'NAVERPAY' | 'SAMSUNGPAY' | 'LPAY' | 'KAKAOPAY' | 'PAYCO' | 'SSG' | 'APPLEPAY' | 'PINPAY' | 'KBPAY' | 'PAYPAL' | 'GCASH' | 'TOUCHNGO' | 'BOOST' | 'BPI' | 'BILLEASE' | 'DANA' | 'ALIPAYHK' | 'TRUEMONEY' | 'RABBIT_LINE_PAY' | 'ALIPAY' | 'SHINHAN' | 'HYUNDAI' | 'SAMSUNG' | 'WOORI' | 'KOOKMIN' | 'LOTTE' | 'NONGHYEOP' | 'HANA' | 'BC' | 'KDBBANK' | 'TOSSBANK' | 'KAKAOBANK' | 'SUHYEOP' | 'JEONBUKBANK' | 'KBANK' | 'POST' | 'SAEMAUL' | 'CITI' | 'SAVINGBANK' | 'JEJUBANK' | 'GWANGJUBANK' | 'SHINHYEOP' | 'JCB' | 'UNIONPAY' | 'MASTER' | 'VISA' | 'DINERS' | 'DISCOVER' | 'IBK_BC' | 'AMEX' | 'TOSS_PAYMENTS' | 'BANKPAY' | 'BRANDPAY' | 'KEYIN';
...
interface WidgetPaymentMethodWidget {
     ...
     */
    on: {
        (eventName: 'paymentMethodSelect', callback: <CustomPaymentMethodCode>(paymentMethod: WidgetSelectedPaymentMethod<CustomPaymentMethodCode>) => void): void;
    };
    /**
     ...
     * @throws {@link PublicError.Widgets.UnknownError} 알 수 없는 오류가 발생한 경우
     */
    getSelectedPaymentMethod: <CustomPaymentMethodCode>() => Promise<WidgetSelectedPaymentMethod<CustomPaymentMethodCode>>;
    /**
...
type RenderAgreement = (params: {
    ...
    selector: string;
    /**
     * 렌더링하고 싶은 약과 UI의 `variantKey`입니다. 상점관리자의 결제위젯 어드민에서 확인할 수 있어요.
     */
    variantKey?: string;
 ...
 * 결제 금액을 변경합니다
 *
 * @param {Amount} amount 새로운 결제 금액입니다.
 *
 * @example
 *  ```javascript
 *  widgets.setAmount({
...
interface CardPaymentRequest extends PaymentRequest {
    ...
    card?: {
        ...
        easyPay?: string;
        /**
         * 5만원 이상의 신용카드 결제에 적용되는 할부 개월 수입니다.
         * 예를 들어, `6`으로 설정하면 할부 개월 수가 6개월로 고정돼요. 자체창에서는 구매자가 할부 개월 수를 볼 수 없으니 사전에 충분히 안내를 해주세요.
         * 0(일시불), 2~12 값으로 설정할 수 있고 `maxCardInstallmentPlan` 파라미터와 함께 사용할 수 없어요.
         */
        cardInstallmentPlan?: number;
        /**
         * 5만원 이상의 신용카드 결제에 적용할 수 있는 최대 할부 개월 수입니다.
         * 예를 들어, `6`으로 설정하면 구매자는 일시불부터 6개월 할부를 선택할 수 있어요. 0(일시불), 2~12 값으로 설정할 수 있고 `cardInstallmentPlan` 파라미터와 함께 사용할 수 없어요.
         */
        maxCardInstallmentPlan?: number;
        /**
         * 5만원 이상의 신용카드 결제에 적용할 수 있는 **상점 부담 무이자** 할부 정보입니다.
         * 구매자가 선택한 카드, 할부 개월 수가 배열에 등록한 정보와 같다면 무이자가 할부가 자동으로 적용돼요.
         */
        ...
        discountCode?: string;
        /**
         * 시간으로 설정하는 결제가 기한입니다. 설정할 수 있는 최대 값은 2160시간(90일)입니다.
         * 기한이 지나고 시도하는 결제는 실패해요. 예를 들어 `24`로 설정하면, 결제 요청 시점으로부터 24시간 동안 결제할 수 있어요.
         */
        validHours?: number;
        /**
         * 특정일로 설정하는 결제 기한입니다. `yyyy-MM-dd'T'HH:mm:ss` ISO 8601 형식입니다.
...
interface BillingAuthRequest {
    /**
     * 자동결제(빌링)에 등록할 결제수단입니다. 토스페이먼츠는 현재 신용·체크카드만 자동결제만 지원해요.
     *
     */
    method: 'CARD';
    /**
...
/**
 *
 * @throws {@link PublicError.PaymentGateway.UserCancelError} 사용자가 결제창을 닫은 경우
 * @throws {@link PublicError.PaymentGateway.InvalidAmountValueError} 결제 금액이 유효하지 않은 경우
 * @throws {@link PublicError.PaymentGateway.InvalidAmountCurrencyError} 결제 금액의 통화가 유효하지 않은 경우
 * @throws {@link PublicError.PaymentGateway.BelowZeroAmountError} 결제 금액이 0 미만인 경우
 * @throws {@link PublicError.PaymentGateway.NotSupportedPromiseError} promise 방식을 지원하지 않는 환경인 경우
 * @throws {@link PublicError.PaymentGateway.NotSupportedMethodError} 가맹점에서 정의되지 않은 method를 넘겨준 경우
 * @throws {@link PublicError.PaymentGateway.IncorrectSuccessUrlFormatError} 가맹점에서 유효하지 않은 successUrl을 넘겨준 경우
 * @throws {@link PublicError.PaymentGateway.IncorrectFailUrlFormatError} 가맹점에서 유효하지 않은 failUrl을 넘겨준 경우
 * @throws {@link PublicError.PaymentGateway.PaymentRequestFailError} 결제에 실패한 경우
 * @throws {@link PublicError.PaymentGateway.InvalidMethodTransactionError} 이미 다른 명령을 수행 중인 경우
 * @throws {@link PublicError.PaymentGateway.InvalidParametersError} 파라미터가 올바르지 않은 경우
 * @throws {@link PublicError.PaymentGateway.UnknownError} 알 수 없는 오류가 발생한 경우
 */
 ...
 * ```
 *
 * @throws {@link PublicError.PaymentGateway.UserCancelError} 사용자가 자동결제 인증창을 닫은 경우
 * @throws {@link PublicError.PaymentGateway.NotSupportedMethodError} 가맹점에서 정의되지 않은 method를 넘겨준 경우
 * @throws {@link PublicError.PaymentGateway.IncorrectSuccessUrlFormatError} 가맹점에서 유효하지 않은 successUrl을 넘겨준 경우
 * @throws {@link PublicError.PaymentGateway.IncorrectFailUrlFormatError} 가맹점에서 유효하지 않은 failUrl을 넘겨준 경우
 * @throws {@link PublicError.PaymentGateway.InvalidMethodTransactionError} 이미 다른 명령을 수행 중인 경우
...
interface TossPaymentsPaymentGateway {
    /**
...
interface TossPaymentsInstance {
     ...
     * ```
     * @param {WidgetSDKInitParams} params 결제위젯 초기화 정보입니다.
     *
     */
    widgets: (params: WidgetSDKInitParams) => TossPaymentsWidgets;
    /**
     ...
     * @param {BrandpaySDKInitParams} params 브랜드페이 초기화 정보입니다.
     *
     */
    brandpay: (params: BrandpaySDKInitParams) => TossPaymentsBrandpay;
    /**
     * 결제창을 초기화합니다. [자세히 >](#결제창)
     *
     ...
     * ```
     * @param {PaymentGatewaySDKInitParams} params 결제창 초기화 정보입니다.
     * @returns 아래 메서드를 호출할 수 있는 결제창 객체를 반환합니다.
     *
     */
    payment: (params: PaymentGatewaySDKInitParams) => TossPaymentsPaymentGateway;
}
/**
 * @example
 * ```javascript
 ...
 */
type TossPayments = (clientKey: string) => TossPaymentsInstance;
interface BrandpaySDKInitParams {
    /**
     * 구매자를 식별하는 고유 아이디입니다.
...
interface PaymentGatewaySDKInitParams {
    /**
     * 구매자를 식별하는 고유 아이디입니다.
     * 이메일・전화번호나 자동 증가하는 숫자와 같이 유추가 가능한 값은 안전하지 않아요. UUID와 같이 충분히 무작위적인 고유 값으로 생성해주세요.
     * 영문 대소문자, 숫자, 특수문자 `-`, `_`, `=`, `.`, `@` 중 최소 1개를 포함하는 최소 2자 이상 최대 50자 이하의 문자열이어야 합니다.
...
export { ANONYMOUS, type WidgetAgreementStatus, type WidgetAgreementWidget, type WidgetPaymentMethodWidget, type WidgetSelectedPaymentMethod, clearTossPayments, loadTossPayments };
